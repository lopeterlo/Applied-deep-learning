	...
	def test(self, test_data):
		...
		for x, id, words_pos, sentence in valid_loader:
			...
			for idx, pred in enumerate(indexs):
				...
				temp += json.dumps({"id":id[idx], "predict_sentence_index": extractive_pred[:2]}) + '\n'
			prediction += temp
			print(id, end ='\r')
		return prediction
...
def main(argv, arc):
	...
	valid = pd.read_json(valid_path, lines= True)
	
	glove_dict = glove()
	# nlp = en_core_web_sm.load()
	nlp = RegexpTokenizer(r'\w+')
	train_dataset = SummaryDataset(train, nlp, glove_dict)
	train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, collate_fn=create_mini_batch, drop_last = True)
	
	valid_dataset = SummaryDataset(valid, nlp, glove_dict)
	valid_loader = torch.utils.data.DataLoader(valid_dataset, batch_size=batch_size, collate_fn=create_mini_batch, drop_last = True)

	model = Extractive_model(input_size = input_size, batch_size = batch_size, hidden_size = hidden_size , epoch = epoch, bidirectional = bidirectional, lr= lr)
	model.fit(train_loader, valid_loader)
	with open(f'./model/model_extractive_0326_2300.pkl', 'wb') as output:

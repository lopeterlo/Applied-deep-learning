...
def testIters(train_path, valid_path, test_path, output_path):
	checkpoint = torch.load('./model/attention_ckpt_2.pt')
...
def test(text_emb, id, dictionary, model, hidden_size, thres_ratio = 0.4):
	prediction = ''
...
#			 print(pred)
		for i in range(batch_size):
			temp = predict.tolist()[0][i]
			temp_word = dictionary.id_to_word[temp]
			order = 1
			val, pred = values.topk(alternative)
			# rule adjust summary
			while temp == 2 or temp_word in ans[i]:
				temp = pred[0][i][order].item()
				temp_word = dictionary.id_to_word[temp]
				order += 1
				if order == alternative :
					break
			pred_word = dictionary.id_to_word[temp]
